...
from console import generate_cli_expect_timeout_list, debug, sleep,spawn_timeout_retry,generate_cli_list,execute_command_via_cli_expect_timeout_list,general_login
...
def telnet_login_via_serial(ip, serial, user, passwd, prompt, login_timeout=2, login_retry_times=20, log_file_path=[], log_file_open=[], is_debug=True):
    ...
    debug('''Telnet login command is "%s"''' % telnet_login_command, is_debug)  
    debug('............Step1 send command to login target via telnet(serial)............',is_debug)
    cli_mode_tuple_list=[(telnet_login_command,'sendline')]
    ...
    if telnet_login_index == 0:
        ...
        if telnet_login_index == 0:
            ...
            is_user=True
            debug('''From 'send enter confirm login' jump to is_user process ''',is_debug)
        elif telnet_login_index == 3:
            is_passwd=True
            debug('''From 'send enter confirm login' jump to is_passwd process ''',is_debug)
        elif telnet_login_index == 4:
            is_no=True
            debug('''From 'send enter confirm login' jump to is_no process ''',is_debug)
        elif telnet_login_index == 5:
            is_prompt=True
            debug('''From 'send enter confirm login' jump to is_prompt process ''',is_debug)
        else:
            print '''Not match any expect in 'step2' expect_list, please check'''
            print 'before is %s, after is %s' % (telnet_login_result.before, telnet_login_result.after)
            telnet_login_result.close(force=True)
            return None
    else:
...
class telnet_host:
    ...
    def execute_command_via_cli_expect_timeout_list(self, spawn_child, cli_expect_timeout_list, ctrl_index_list=[], cli_retry_times=2):
        ...
        telnet_cli_result=execute_command_via_cli_expect_timeout_list(spawn_child, cli_expect_timeout_list, cli_retry_times,wait_time, ctrl_index_list, is_debug)        
...
def telnet_execute_cli(ip, user, passwd, serial, cli_list, prompt, timeout, retry_times, logdir, logfile, is_debug, is_shell, shellpasswd, wait_time, bootpasswd=[]):
    ...
    if telnet_host_login:
        ...
        telnet_host_execute = telnet.execute_command_via_cli_expect_timeout_list(telnet_host_login, cli_expect_timeout_list, ctrl_index_list,cli_retry_times)                                       
    else:
