    ...
    def login_via_ip(self,login_ip_timeout=2):
        ...
        ###Welcome to Aerohive product
            ...
            if index == 0:
                ...
                index = telnet_login_result.expect([pexpect.TIMEOUT, '[Pp]assword', 'Use the Aerohive.*<yes|no>:', 'Aerohive Networks Inc.*#'], timeout=login_ip_timeout)
                if index == 0:
...
#                ###can login enable mode automatically
...
#                    telnet_login_result.sendline(passwd)
#                    index = telnet_login_result.expect([pexpect.TIMEOUT, '[Pp]assword', '\w+#'], timeout=login_ip_timeout)
#                    if index == 0:
...
#            debug('''Step2 send user name '%s' to confirm login''' % user, is_debug)
            ...
            index = telnet_login_result.expect([pexpect.EOF, pexpect.TIMEOUT, prompt, 'Use the Aerohive.*<yes|no>:','login',], timeout=login_serial_timeout)
            if index == 0:
                ...
                if index == 0:
                    ...
                    index = telnet_login_result.expect([pexpect.TIMEOUT, 'login', prompt],timeout=login_serial_timeout)
                    if index == 0:
...
def telnet_execute_cli(ip,user,passwd,cli_list,serial,timeout,prompt,logdir,logfile,is_debug,is_shell,shellpasswd):
    ...
    ###Check if '_shell' in clilist
        if '_shell' in cli_list:
            ...
            ######if no, should logout automatically
            else:
                ...
                for cli_index in range(len(cli_list)):
                    if cli_index < shell_index:
                        ...
                        if index == 0:
                            ...
                            telnet_host_execute=telnet.execute_command(cli_list[cli_index], '\w+:~\$', timeout, telnet_host_login)
            ### logout the shell mode automatically
...
parse.add_argument('-o', '--timeout', required=False, default='5', dest='timeout',
