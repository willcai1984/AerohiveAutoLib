...
from console import generate_cli_expect_timeout_list
...
class telnet_host:
    ###v13 add wait time
    def __init__(self, ip, user, passwd, serial='', is_debug=False, absolute_logfile='', prompt='[$#>]',wait_time=0):
        self.ip=ip
        self.user=user
        self.passwd=passwd
    ...
    def execute_command_via_cli_expect_timeout_list(self, cli_expect_timeout_list, spawn_child, ctrl_index_list=[]):
        ...
        for cli, cli_expect, timeout in cli_expect_timeout_list:
            if cli_expect_timeout_list.index((cli, cli_expect, timeout)) in ctrl_index_list:
                ...
                telnet_cli_result.sendcontrol(ctrl_cli)
            else:
                debug('Send normal command', is_debug)
                telnet_cli_result.sendline(cli)
            ###2 may meet some display command cannot show all the parameters,need send '' to continue
            index = telnet_cli_result.expect([pexpect.TIMEOUT, cli_expect, '--More--', 'More:', '-- More --'], timeout=timeout)
            if index == 0:
                print '''TimeOut when execute CLI, fail in Execute CLI parter'''
                print 'CLI is %s, Expect is %s, Timeout is %s' % (cli, cli_expect, timeout)
                print 'before is %s' % (telnet_cli_result.before)
                print 'after is %s' % (telnet_cli_result.after)
                telnet_cli_result.close(force=True)
                return None
            elif index == 1:
...
#                    telnet_cli_result.send(' ')
                    ### v8 modify blank to enter
                    more_index += 1
                    telnet_cli_result.sendline('')
                    index = telnet_cli_result.expect([cli_expect, '--More--'], timeout=timeout)
...
###v13 add wait_time
def telnet_execute_cli(ip,user,passwd,cli_list,serial,timeout,prompt,logdir,logfile,is_debug,is_shell,shellpasswd,wait_time):
    timeout=int(timeout)
    #v13 modify wait_time str to float
    ...
    ###set class
    ###v13 add wait time
    telnet=telnet_host(ip, user, passwd, serial, is_debug, absolute_logfile,prompt,wait_time)
    ###login
    ...
    if telnet_host_login:
        execute_cli_list = generate_cli_expect_timeout_list(cli_list, prompt, timeout, passwd, shellpasswd)
        ...
        debug(ctrl_index_list,is_debug)
        telnet_host_execute = telnet.execute_command_via_cli_expect_timeout_list(cli_expect_timeout_list, telnet_host_login, ctrl_index_list)                                       
    else:
...
#    debug(args,is_debug)
    ...
    try:
        ...
        telnet_result=telnet_execute_cli(ip,user,passwd,execute_cli_list,serial,timeout,prompt,logdir,logfile,is_debug,is_shell,shellpasswd,wait_time)
    except Exception, e:
        print str(e)
    else:
