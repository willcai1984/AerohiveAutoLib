...
class telnet_host:
    ...
    def login_via_ip(self,login_ip_timeout=2):
        ...
        index = telnet_login_result.expect([pexpect.TIMEOUT, 'No route to host', 'Welcome to Aerohive Product.*login:', 'Connected.*Escape character.*Password:','Connected.*User Name:'], timeout=login_ip_timeout)
        if index  == 0:
...
#            debug('''Step2 send user name '%s' to confirm login''' % user, is_debug)
    ...
    def logout_via_ip(self,spawn_child,logout_ip_timeout=5):
        telnet_login_result=spawn_child
