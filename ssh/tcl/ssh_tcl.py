...
def main():
    ...
    debug('Type in cli list is as below...')
    debug(cli_list,is_debug)
    debug('sys.argv is as below...')
    debug(sys.argv,is_debug)
    execute_cli_list=[]
    if config_file_path:
        ...
        for v_index in v_argv_list:
            if v_index < f_argv_list[0]:
                ...
                if is_first_more:
                    ...
                    execute_cli_list.append(para_list[v_index+1])
        ### tcl \$ to \\$
        execute_cli_list=[re.sub('\$','\\$',i) for i in execute_cli_list]
    else:
        ###tcl \$ to \\$
        for cli in cli_list:
            execute_cli_list.append(re.sub('\$','\\$',cli))
