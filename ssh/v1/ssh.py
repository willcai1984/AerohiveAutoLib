...
###Use global variable need add class name  'ssh_host.login_prompt'
        index=ssh_login_result.expect([pexpect.TIMEOUT,'[Pp]assword',prompt], timeout=20)
        if index==0:
            ...
            debug('Login successfully, can execute cli now')
    ...
    def logout(self,spawn_child,logout_timeout=10):
        ...
        if index== 0:
            print '''TimeOut when send ctrl+d to logout'''
