...
#                    console_cli_result.send(' ')
    ...
    def logout(self, spawn_child, logout_timeout=10):
        ...
        # ##v12 add quit in specail cli process
...
#        ###v12 add bootload quit
#        index = console_logout_result.expect([pexpect.TIMEOUT, 'want to save it.*', 'login:.*', 'Unrecognized command.*', 'Unknown command'], timeout=logout_timeout)
#        if index == 0:
...
#        elif index == 4:
#            debug('Meet bootload status,only free console', is_debug) 
...
def console_execute_cli(ip, serialname, user, passwd, cli_list, timeout, prompt, logdir, logfile, is_debug, is_shell, shellpasswd, retry_times, wait_time, bootpasswd):
    ...
    if console_host_login:
        # ##v7
        # ##v12
        execute_cli_list = generate_cli_list(cli_list, prompt, timeout, passwd, shellpasswd, bootpasswd)
