...
class console_host:
    ...
    def login(self,login_timeout=5):
        ip=self.ip
        ...
        if index  == 0:
            ...
            ###should send '' twice to confirm we can enter to meet the login:
#            time.sleep(0.5)
...
#            time.sleep(0.5)
            ...
            index = console_login_result.expect([pexpect.EOF, pexpect.TIMEOUT, prompt, 'Use the Aerohive.*no>:','Welcome to Aerohive Product.*login','[Pp]assword'], timeout=login_timeout)
            if index == 0:
...
def console_execute_cli(ip,serialname,user,passwd,cli_list,timeout,prompt,logdir,logfile,is_debug,is_shell,shellpasswd):
    ...
    ###login
    console_host_login=console.login()
    ###execute cli
...
parse.add_argument('-o', '--timeout', required=False, default='10', dest='timeout',
...
###check is config_file_path is null, if yes, use cli_list,if no, use config_file_path
    ...
    try:
        console_result=console_execute_cli(ip,serialname,user,passwd,execute_cli_list,timeout,prompt,logdir,logfile,is_debug,is_shell,shellpasswd)
    except Exception, e:
        print str(e)
    else:
