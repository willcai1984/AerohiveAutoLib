...
def send_command(command,spawn_child,is_debug=True,para_wait_time=0.1,send_mode='sendline'):
    ...
    ### more than 60, send the char each and send enter in the end
        debug('The cli is more than 60, send parameters separatly',is_debug)
        for char in command:
            spawn_child.send(char)
            sleep(para_wait_time)
        if send_mode=='sendline':
            ...
            debug(spawn_child.after,is_debug)
            spawn_child.sendline('')
