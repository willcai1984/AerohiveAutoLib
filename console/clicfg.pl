...
my %specialTbl  = (
	...
	"clicfg*" => {
		...
		"data"     => "0",
		"retry"    => 30,
		"prompt"   => [" Cli configuration passed"],
		"response" => ["y"],
		"notmo"    => 0,
	},
	"reset config *" => {
	...
	'boot-param country-code\s*\d' => {
		"function" => "waitForLogin",
		"tmo"      => 60,
		"system"   => "no",
		"data"     => "0",
		"retry"    => 5,
		"prompt"   => ['To apply radio setting for the new country code *'],
		"response" => ["Y"],
		"notmo"    => 0,
	},
	...
	"exit\$" => {
		...
		"data"     => "0",
		"retry"    => 5,
		"prompt"   => ["Please press Enter to activate this console*|Welcome back to CLI console"],
		"response" => ["y"],
		"notmo"    => 1,
	},
	"^perl -MCPAN" => {
		...
		@buffer2 = $localExp->expect(
			...
			[
				...
				sub {
					$rc = $PASS;
					my $fh = shift;
					print " ---  CLI  config change--- \n" if ( $userInput->{debug} > 0 );
					$errorCode = $CLI_PROMPT;
					if ($userInput->{notsaveconfig} == 1) {
					print " ---  CLI  config change--3- \n" if ( $userInput->{debug} > 0 );
						$fh->send("N\n");
					} else {
					print " ---  CLI  config change--4- \n" if ( $userInput->{debug} > 0 );
						$fh->send("Y\n");
					}
				  }
			],
...
sub specialCmdProcess {
	...
	} elsif ( $entry->{system} =~ /no/ ) {
		$rc = $localExp->send("$com\n");
		print "rc.send $com=$rc\n";
	} elsif ( $entry->{system} =~ /ctrl/ ) {
		if ( $com =~ /ctrl-c/ ) {
			$rc = $localExp->send("\cC");
			print "rc.send $com\n";
		} else {
			$msg = "ctrl value ($com) should be in (ctrl-c,..)";
			print $msg;
			$rc = $FAIL;
			return ( $rc, $msg );
		}
	} else {
		$msg = "system values should be in (yes, no, ctrl)";
		print $msg;
		$rc = $FAIL;
		return ( $rc, $msg );
	}
...
sub waitForLogin {
	...
	while ( $a < 4 ) {
		$localExp->expect(
			...
			[
				qr 'Maximum login tries exceeded. Go away!',
				sub {
					my $fh = shift;
					$rc      = $FAIL;
					$a       = 20;
					$tmoFlag = 0;
					print " Maximum login tries exceeded!\n";
					return;
				  }
			],
			[
				...
				sub {
					...
					my $fh = shift;
					if ($userInput->{notsaveconfig} == 1) {
					print " ---  CLI  config change--1- \n" if ( $userInput->{debug} > 0 );
						$fh->send("N$LIMITER");
					} else {
					print " ---  CLI  config change-2- \n" if ( $userInput->{debug} > 0 );
						$fh->send("y$LIMITER");
					}
					#			       $fh->send("Y$LIMITER");
			...
			[
				...
				sub {
					...
					if ( $userInput->{bootscreen} == 0 ) {
						...
						#sleep 10s to wait reset take effect and pass the "stop boot"
						sleep 10;
						$a = 1;
						return;
					}
...
sub exit_process {
	my ( $localExp, $userInput, $msg_info ) = @_;
	...
	if ( $userInput{logfile} !~ $NO_FILE ) {
		my $rc;
		$rc = `tr -d "\r" <$locallog >$log_file`;
		$rc = `sed -i "s/                            //g" $log_file`;
		print("\n==> $msg_info. Log is saved to $log_file\n");
	} else {
		print("\n==> $msg_info. Log is saved to $locallog\n");
	}
}
...
$rc = GetOptions(
	...
	"nosave" => \$userInput{notsaveconfig},
);
...
$temp = $userInput{port};
SWITCHMAIN:
my $conRefused = 1;    ##jyu. change from 2 to 1
my $conRetry   = 0;
my $conWait    = 2;
my $doneFlag   = 1;
...
while ($doneFlag) {
	...
	if ( $userInput{logfile} !~ $NO_FILE ) {
		$log_file = $userInput{logdir} . "/" . $userInput{logfile};
		$locallog = $log_file . ".bak";
	} else {
	...
	if ( $rc != $PASS ) {
		...
		if ( $junk[1] == 20 ) {
			if ( $conRetry < $conRefused ) {
				$conRetry += 1;
				print "waitForLogin failed! Wait for $conWait seconds $conRetry\n";
				sleep $conWait;
				$rc = $PASS;
				next;
			}
		...
		## waitForLogin failed process (exit console and copy log)
		exit_process( $exp, \%userInput, "waitForLogin failed" );
...
if ( $rc != $PASS ) {
	...
	## sendBatchCmd failed process (exit console and copy log)
	exit_process( $exp, \%userInput, "sendBatchCmd failed" );

	exit(1);
}

## Success process (exit console and copy log)
exit_process( $exp, \%userInput, "sendBatchCmd success" );
...
[B<-nosave> I<optinal exit without saving config>]
