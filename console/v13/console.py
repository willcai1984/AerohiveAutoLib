...
def console_login(ip,user,passwd,serialname,prompt,log_file_path=[],log_file_open=[],retry_times=10, login_timeout=5,is_debug=True):
    ###init parameters process
    retry_times=int(retry_times)
    ###v13 retry_times should be more than 3 due to the retry format of 'Reboot after bootload status or normal(no reboot) status' 
    if retry_times < 3:
    ...
    retry_cli_mode_tuple_list=[[('','sendline')]]*retry_times
#    spawn_timeout_retry(cli_mode_tuple_list, expect_list, timeout=5 , retry_cli_mode_tuple_list=[], retry_times=5, spawn_child='', is_debug=True)
    console_login_info=spawn_timeout_retry(cli_mode_tuple_list, expect_list, timeout, retry_cli_mode_tuple_list , retry_times, '', is_debug)
    ...
    if console_login_index == 0:
        ...
        if is_reboot_before_bootload:
            ...
            if console_login_result.after == 'bumped' or console_login_result.after == 'WARNING':
                ...
                timeout=login_timeout
                retry_cli_mode_tuple_list=[[('','sendline')]]*retry_times
                console_login_info=spawn_timeout_retry(cli_mode_tuple_list, expect_list, timeout, retry_cli_mode_tuple_list , retry_times, console_login_result, is_debug)
                console_login_result=console_login_info[0]
                console_login_index=console_login_info[1]
                if console_login_index == 0:
                ...
                expect_list=[pexpect.TIMEOUT, 'login.*', '[Pp]assword.*','yes\|no>:.*', prompt]
                timeout=login_timeout
                retry_cli_mode_tuple_list=[[('','sendnone')]]*retry_times
                console_login_info=spawn_timeout_retry(cli_mode_tuple_list, expect_list, timeout, retry_cli_mode_tuple_list , retry_times, console_login_result, is_debug)
                console_login_result=console_login_info[0]
                console_login_index=console_login_info[1]
                if console_login_index == 0:
            ...
            retry_cli_mode_tuple_list=[[('','sendline')],[('','sendline')],[('e','sendcontrol'),('co','send')]]+[[('','sendline')]]*(retry_times-3)
            console_login_info=spawn_timeout_retry(cli_mode_tuple_list, expect_list, timeout, retry_cli_mode_tuple_list , retry_times, console_login_result, is_debug)
            console_login_result=console_login_info[0]
            console_login_index=console_login_info[1] 
            if console_login_index == 0:
    ...
    for cli, cli_expect, cli_timeout in cli_expect_timeout_list:
        ...
        if index == 0:
            ...
            while more_index == 1:
                ...
                timeout=cli_timeout
                retry_times=cli_retry_times
                retry_cli_mode_tuple_list=[[('','sendline')]]*retry_times
                more_info=spawn_timeout_retry(cli_mode_tuple_list, expect_list, timeout, retry_cli_mode_tuple_list , retry_times, execute_cli_result, is_debug)
...
def console_logout(spawn_child, logout_timeout=10, is_debug=True):
...
class console_host:
    ...
    def login(self, login_timeout=2,login_retry_times=5):
        ...
        console_login_result=console_login(ip,user,passwd,serialname,prompt,log_file_path,log_file_open,login_retry_times, login_timeout,is_debug)            
        return console_login_result
    ...
    def execute_command_via_cli_expect_timeout_list(self, spawn_child, cli_expect_timeout_list, ctrl_index_list=[], cli_retry_times=2):
        ...
        execute_command_result=execute_command_via_cli_expect_timeout_list(spawn_child,cli_expect_timeout_list, cli_retry_times, wait_time, ctrl_index_list, is_debug)
    ...
    def logout(self, spawn_child, logout_timeout=10):
        is_debug = self.is_debug
        console_logout_result=console_logout(spawn_child, logout_timeout, is_debug)
        return console_logout_result
...
def console_execute_cli(ip, serialname, user, passwd, cli_list, timeout, prompt, logdir, logfile, is_debug, is_shell, shellpasswd, retry_times, wait_time, bootpasswd):
    timeout = int(timeout)
    retry_times = int(retry_times)
    ...
    console_host_login = console.login(login_timeout,retry_times)
    if console_host_login:
        execute_cli_list = generate_cli_expect_timeout_list(cli_list, prompt, timeout, passwd, shellpasswd, bootpasswd)
...
def main():
    ...
    try:
        console_result = console_execute_cli(ip, serialname, user, passwd, execute_cli_list, timeout, prompt, logdir, logfile, is_debug, is_shell, shellpasswd, retry_times, wait_time, bootpasswd)
    except Exception, e:
        print str(e)
    else:
