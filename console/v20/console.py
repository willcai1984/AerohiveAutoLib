...
def send_command(command, spawn_child, is_debug=True, para_wait_time=0.1, send_mode='sendline', buffer_sleep_time=5):
    buffer_sleep_time = int(buffer_sleep_time)
    ### less than 70, sendline directly
    if send_mode == 'sendline':
        ...
        return None
    ###v20 modify from 60 to 70
    ###v19 add sleep for expect moudle to eat all buffer
    if len(command) < 70:
        pass
    else:
        ### more than 70, send the char each and send enter in the end
        ###v18 send enter twice and clear the buffer
        ###clear buffer
        debug('sleep %ss to eat all buffer' % buffer_sleep_time, is_debug)
        sleep(buffer_sleep_time)
        spawn_child.expect('.*')
        debug('send enter first time, and clear buffer', is_debug)
...
#    debug('............Timeout_retry func Execute command send process............', is_debug)
    ...
    debug('''Match expect index is %s, vaule is '%s' ''' % (retry_index, spawn_child.after))
...
#            print 'Meet login again, user or password maybe incorrect, please check'
            ###v20
            ###retry 5 times for getting the user/passwd error on Aerohive product
            ###define private para
            passwd_retry_index=1
            passwd_retry_times=5
            passwd_retry_num=0
            spawn_child=general_login_result
            passwd_retry_timeout=20
            while passwd_retry_index == 1:
                if passwd_retry_num >= passwd_retry_times*2:
                    print 'Retry username and passwd %s times and still failed, please check the username and password' % passwd_retry_times
                    print 'before is %s, after is %s' % (spawn_child.before, spawn_child.after)
                    spawn_child.close(force=True)
                    return None
                real_retry_num,real_retry_index=divmod(passwd_retry_num, 2)          
                if real_retry_index == 0:
                    debug('Retry username and passwd %s time begin' % (int(real_retry_num)+1), is_debug)
                    spawn_child.sendline(user)
                    passwd_retry_index = spawn_child.expect([pexpect.TIMEOUT, '[Pp]assword.*'],timeout=passwd_retry_timeout)
                elif real_retry_index == 1:
                    spawn_child.sendline(passwd)
                    passwd_retry_index = spawn_child.expect([pexpect.TIMEOUT, 'login.*:','Maximum .*Go away.*','yes\|no>:.*', prompt],timeout=passwd_retry_timeout)
                    debug('Retry username and passwd %s time over' % (int(real_retry_num)+1), is_debug)
                passwd_retry_num += 1
            if passwd_retry_index==0:
                print 'Timeout when retrying username password %s times' % real_retry_num
                spawn_child.close(force=True)
                return None
            elif passwd_retry_index==2:
                print 'Reach the max Aerohive products retry times, user or password is incorrect, please check'
                spawn_child.close(force=True)
                return None
            elif passwd_retry_index==3:
                is_no = True
                debug('''From is_passwd jump to is_no process ''', is_debug)
            elif passwd_retry_index==4:
                is_prompt = True
...
def generate_cli_expect_timeout_list(cli_list, prompt, timeout, passwd='', shellpasswd='', bootpasswd=''):
    ...
    for cli in cli_list:
        ...
        # ##v9 add support save image and reboot
            ...
            ###v19 add is shell check
            if shellpasswd:
                cli_expect_timeout_list.append((cli, prompt, timeout))                
                ctrl_index = ctrl_index + 1
            else:
                ###v19 default is save config and exit
                cli_expect_timeout_list.append(('save config', prompt, timeout))
...
#    spawn_timeout_retry(cli_mode_tuple_list, expect_list, timeout=5 , retry_cli_mode_tuple_list=[], login_retry_times=5, spawn_child='', is_debug=True)
    ...
    if console_login_index == 0:
        ...
        if is_reboot_before_bootload:
            ...
            retry_cli_mode_tuple_list = [[('', 'sendnone')]] * 10 + [[('e', 'sendcontrol'), ('co', 'send')], [('', 'sendline')]] + [[('', 'sendnone')]] * (login_retry_times - 12)
            console_login_info = spawn_timeout_retry(cli_mode_tuple_list, expect_list, timeout, retry_cli_mode_tuple_list , login_retry_times, console_login_result, is_debug)
            console_login_result = console_login_info[0]
            console_login_index = console_login_info[1] 
            if console_login_index == 0:
...
def execute_command_via_cli_expect_timeout_list(spawn_child, cli_expect_timeout_list, cli_retry_times=2, wait_time=0.5, ctrl_index_list=[], is_debug=True):
    ...
    cli_num = 1
    ###v18 add para wait time for more than 70 command
    para_waite_time = 0.1
    for cli, cli_expect, cli_timeout in cli_expect_timeout_list:
        if cli_expect_timeout_list.index((cli, cli_expect, cli_timeout)) in ctrl_index_list:
            ...
            ###v18
            send_command(ctrl_cli, execute_cli_result, is_debug, para_waite_time, 'sendcontrol')
        else:
            debug('%s send normal command' % cli_num, is_debug)
            ###v18
            send_command(cli, execute_cli_result, is_debug, para_waite_time)
